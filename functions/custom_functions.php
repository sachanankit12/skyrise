...
{
	$generalinfo = get_option('mysite_general_settings');
	if($generalinfo['related_property'])
	{
		return $generalinfo['related_property'];
	}else
	{
		return 'Location';
	}
...
function add_to_favorite($property_id)
{
	...
	$user_meta_data = get_usermeta($current_user->data->ID,'user_favorite_property');
	$user_meta_data[]=$property_id;
	update_usermeta($current_user->data->ID, 'user_favorite_property', $user_meta_data); 	
	echo 'Favorited <a href="javascript:void(0);" class="addtofav" onclick="javascript:addToFavorite(\''.$property_id.'\',\'remove\');"><b>(remove)</b> </a>';
...
function favourite_html($user_id,$post_id)
{
	global $current_user;
			...
			$user_meta_data = get_usermeta($current_user->data->ID,'user_favorite_property');
									...
									?>
									<span id="favorite_property_<?php echo $post_id;?>" class="fav"><a href="javascript:void(0);" class="addtofav"  onclick="javascript:addToFavorite(<?php echo $post_id;?>,'add');"><?php _e(ADD_TO_FAVOURITE_TEXT);?></a></span>
                                     <?php } 
									 //}
}
...
function custom_list_authors($args = '',$params = array()) {
	...
	$sub_cat = "select count(p.ID) from $wpdb->posts p where p.post_author=u.ID and p.post_status='publish' and p.post_type='post' $sub_cat_sql";
	if($params['sort']=='most')
	{
	...
	{
		//$sql = "SELECT u.ID, u.user_nicename from $wpdb->users u join $wpdb->usermeta um on um.user_id=u.ID where 1 and ($sub_cat)>0 " . ($exclude_admin ? "and  u.user_login <> 'admin' " : '');
...
?>	<li <?php if(in_category(get_cat_id_from_name(get_option('ptthemes_featuredcategory')))){ echo 'class="featured"';}?>><?php  is_new_property( $post->ID );?>
...
<a href="<?php the_permalink();?>">
					...
					{
					?>
					<img src="<?php echo $thumb_img; ?>" height="109" width="164" alt="" title="" />
					<?php
					}else
					{
					?>
					...
					<div class="content">
                       ...
                       <p> <span class="field"> <?php _e(BEDS_TEXT);?> </span> <span>:  <?php echo get_post_meta($post->ID,'bed_rooms',true);?><?php //echo $cat_info_arr['bed']['name'];?> </span></p> 
                        ...
                        <div class="property_detail"> 
                        <p> <span class="field"> <?php _e(PRO_MLS_TEXT);?> </span> <span>: <?php echo get_post_meta($post->ID,'mlsno',true);?> </span> </p> 
                       <p> <span class="field"> <?php _e(PRO_PROPERTY_ID_TEXT);?> </span> <span>:  <?php echo $post->ID;?> </span></p> 
                        ...
                        <p class="propertylistinglinks">
 						  <span class="agent"> <?php _e(AGENT_TITLE); ?> : <a href="<?php echo get_author_link($echo = false, $post->post_author, $author_nicename = '');?>" class="emailagent"><?php echo get_the_author_meta( 'display_name',$post->post_author);?> </a> </span>
							  
                        <span><a href="<?php the_permalink();?>"><?php _e(VIEW_MORE_DETAILS_TEXT);?> &raquo;</a></span>
...
?>
	...
	<b class="sale"> <?php _e(PRO_FOR_TEXT);?> <?php _e(get_post_meta($pid,'property_type',true));?> : </b> <?php echo get_property_price($pid);?><b> 
...
//NEW FUNCTIONS//
function get_user_logo_pic($user_id,$height=100,$width=100)
{
	 $user_data = get_userdata(intval($user_id));
	 if($user_data ->user_logo !='')
	 {    
	     $img_data =  '<img name="user_photo" class="agent_photo" id="user_photo" src="'.$user_data ->user_logo.'" width="'.$width.'" height="'.$height.'"  />';
      }
	  /*else{
		  $img_data =  '<img name="user_photo" class="agent_photo" id="user_photo" src="'.get_bloginfo('template_directory').'/images/logo_post.png" width="'.$width.'" height="'.$height.'"  />';
	 } */
	 return $img_data;
}
